...
class MockMatchingService implements MatchingService {
  static const _companies = [
    CompanyMatch(id:'c1', name:'Tech Innovate', type:'Empresa', location:'Sao Paulo, SP',
      description:'Empresa de tecnologia especializada em solucoes IoT e automacao industrial.',
      openRoles:['Dev Flutter Junior','Dev Backend Dart','Estagiario Mobile'],
      techStack:['Flutter','Dart','MQTT','PostgreSQL','React'],
      matchPercent:0, matchReason:''),
    CompanyMatch(id:'c2', name:'Startup Nova', type:'Startup', location:'Remoto',
      description:'Startup de educacao digital buscando desenvolvedores apaixonados por impacto social.',
      openRoles:['Dev Full Stack Junior','UX/UI Designer','Dev Mobile'],
      techStack:['React Native','Node.js','Firebase','TypeScript','Flutter'],
      matchPercent:0, matchReason:''),
    CompanyMatch(id:'c3', name:'DataSolutions', type:'Empresa', location:'Campinas, SP',
      description:'Empresa de analytics e BI com projetos de grande escala para o varejo.',
      openRoles:['Analista de Dados Junior','Dev Backend Python','Estagiario BD'],
      techStack:['Python','MySQL','PostgreSQL','Power BI','SQLite'],
      matchPercent:0, matchReason:''),
    CompanyMatch(id:'c4', name:'WebCraft Agency', type:'Agencia', location:'Rio de Janeiro, RJ',
      description:'Agencia digital criando experiencias web para marcas lideres do mercado.',
      openRoles:['Dev Frontend Junior','Dev Vue.js','Estagiario Web'],
      techStack:['Vue.js','React','Node.js','TypeScript','CSS'],
      matchPercent:0, matchReason:''),
    CompanyMatch(id:'c5', name:'CloudBridge', type:'Empresa', location:'Remoto',
      description:'Empresa de infraestrutura cloud ajudando startups a escalar suas plataformas.',
      openRoles:['Dev Backend Junior','DevOps Estagiario','Analista de Sistemas'],
      techStack:['Node.js','Python','MySQL','Docker','Laravel'],
      matchPercent:0, matchReason:''),
    CompanyMatch(id:'c6', name:'MobileFirst', type:'Startup', location:'Florianopolis, SC',
      description:'Startup focada em apps mobile de alta performance para o setor financeiro.',
  ...
  Future<List<CompanyMatch>> findMatches(List<String> technologies) async {
    ...
    final scored = _companies.map((c) {
      final compTechs = c.techStack.map((t) => t.toLowerCase()).toList();
      final matches = techs.where((t) =>
          compTechs.any((ct) => ct.contains(t) || t.contains(ct))).length;

      if (matches == 0) return null;

      final pct = ((matches / techs.length) * 100).clamp(20, 98).round();
      final matchedNames = techs
          .where((t) => compTechs.any((ct) => ct.contains(t) || t.contains(ct)))
          .map((t) => technologies.firstWhere(
              (orig) => orig.toLowerCase() == t, orElse: () => t))
          .take(2)
          .join(' e ');
      final reason = 'Voce usa $matchedNames, tecnologias que esta empresa busca ativamente.';

      return CompanyMatch(
        id: c.id, name: c.name, type: c.type,
        location: c.location, description: c.description,
        openRoles: c.openRoles, techStack: c.techStack,
        matchPercent: pct, matchReason: reason,
      );
    }).whereType<CompanyMatch>().toList();
